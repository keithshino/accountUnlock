...
interface RequestFormProps {
  onAddTasks: (task: { requesterName: string; requesterEmail: string; employees: { employeeName: string; employeeId: string }[] }) => void;
}
...
const RequestForm: React.FC<RequestFormProps> = ({ onAddTasks }) => {
  ...
  const validate = () => {
    ...
    if (!requesterName.trim()) newErrors.requesterName = '依頼者氏名は必須です。';
    ...
    const employeeErrors = employees.map(emp => {
      ...
      return Object.keys(singleEmployeeErrors).length > 0 ? singleEmployeeErrors : undefined;
    });
  ...
  const handleConfirmSubmit = () => {
    onAddTasks({
      requesterName,
      requesterEmail,
      employees: employees.map(emp => ({ employeeName: emp.name, employeeId: emp.id })),
    });
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg">
        <form onSubmit={handleSubmit} noValidate>
          ...
          <fieldset>
            ...
            <div className="space-y-4">
              {employees.map((employee, index) => (
                <div key={employee.key} className="flex items-start gap-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  {employees.length > 1 && (
                    <button type="button" onClick={() => handleRemoveEmployee(employee.key)} className="mt-8 p-2 rounded-full hover:bg-red-100 group" aria-label="Remove employee">
                      <TrashIcon />
                    </button>
                  )}
                </div>
            ...
            <div className="mt-4">
              <button type="button" onClick={handleAddEmployee} className="flex items-center gap-2 text-blue-600 font-semibold py-2 px-3 rounded-md hover:bg-blue-50 transition-colors">
                <PlusIcon />
                <span>社員を追加</span>
              </button>
            </div>
          ...
          <div className="mt-8 text-center">
            <button type="submit" className="w-full sm:w-auto bg-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-transform transform hover:scale-105 shadow-md">
              依頼を提出する
            </button>
          </div>
        </form>
